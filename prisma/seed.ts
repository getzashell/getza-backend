import { Jurisdiction, OutputFormat, PrismaClient, ProductStatus, ProductType } from '@prisma/client';
...
type SeedProduct = {
  ...
  category: string;
  subcategory?: string;
  descriptionRte?: Record<string, any>;
  type: ProductType;
  ...
  for (const seed of products) {
    const product = await prisma.product.upsert({
      ...
      update: {
        ...
        type: seed.type,
        status: seed.status || ProductStatus.PUBLISHED,
      },
      create: {
        slug: seed.slug,
        title: seed.title,
    ...
    for (const tmpl of seed.templates) {
      const template =
        (await prisma.template.findFirst({ where: { productId: product.id, name: tmpl.name } })) ||
